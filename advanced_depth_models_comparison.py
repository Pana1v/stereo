...
class AdvancedDepthModels:
    ...
    def load_dpt_hybrid(self):
        """Load DPT-Hybrid model (combines ViT with CNN features)"""
        try:
            print("Loading DPT-Hybrid model...")
            processor = DPTImageProcessor.from_pretrained("Intel/dpt-hybrid")
            model = DPTForDepthEstimation.from_pretrained("Intel/dpt-hybrid").to(self.device)
            self.models['DPT-Hybrid'] = model
            self.processors['DPT-Hybrid'] = processor
            print("✓ DPT-Hybrid loaded successfully")
            return True
        except Exception as e:
            print(f"✗ Failed to load DPT-Hybrid: {e}")
            return False
    
    def load_midas_small(self):
        """Load MiDaS Small as baseline"""
        try:
            print("Loading MiDaS Small model...")
            processor = DPTImageProcessor.from_pretrained("Intel/dpt-small")
            model = DPTForDepthEstimation.from_pretrained("Intel/dpt-small").to(self.device)
    ...
    def run_comparison(self, image_path):
        ...
        models_loaded += self.load_dpt_hybrid()
